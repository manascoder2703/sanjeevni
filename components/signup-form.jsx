...
export function SignupForm({ className, role = 'patient', ...props }) {
  const [loading, setLoading] = useState(false);
  const { register, pendingOTP } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.otpRequired) {
        // OTPModal appears automatically via pendingOTP in AuthContext
                      ...
                      <Select onValueChange={(val) => setForm(prev => ({ ...prev, specialization: val }))} required>
                        ...
                        <SelectContent className="bg-black/40 backdrop-blur-xl border-white/10 text-white min-w-[200px]">
                          {specializations.map(s => (
                            <SelectItem key={s} value={s} className="focus:bg-white focus:text-black transition-colors duration-200 cursor-pointer rounded-lg mx-1 my-0.5">{s}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    </Field>

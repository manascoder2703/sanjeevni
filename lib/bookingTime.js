...
export function parseTimeLabelToMinutes(timeLabel) {
  ...
  const thirtyMinMatch = trimmed.match(/^(\d{1,2})\s?(AM|PM)$/);
  if (thirtyMinMatch) {
    let hours = Number(thirtyMinMatch[1]);
    const modifier = thirtyMinMatch[2];
    if (hours === 12) hours = 0;
    if (modifier === 'PM') hours += 12;
    return hours * 60;
  }
...
export function parseDateAndTimeSlot(date, timeSlot) {
  ...
  if (dashMatch) {
    ...
    // Try DD-MM-YYYY
    const reverseDashMatch = date.match(/^(\d{1,2})-(\d{1,2})-(\d{4})$/);
    if (reverseDashMatch) {
      day = Number(reverseDashMatch[1]);
      month = Number(reverseDashMatch[2]);
      year = Number(reverseDashMatch[3]);
    } else {
      // Fallback: try standard New Date() parsing for other strings (e.g. "28 April 2026")
  ...
  // Use ISO format with +05:30 offset

...
export async function sendOTPEmail(email, otp, name) {
  await transporter.sendMail({
    ...
    to: email,
    subject: 'Your Sanjeevni Verification Code',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background:#f4f4f4; padding: 20px;">
        <h2>Verify your email</h2>
        <p>Hi ${name},</p>
        <p>Use the following code to complete your login/registration. This code will expire in 10 minutes.</p>
        <div style="background:#fff; padding: 20px; text-align:center; font-size:32px; letter-spacing:8px; font-weight:bold; border-radius:10px; margin:20px 0;">
          ${otp}
        </div>
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background:#f4f4f4; padding: 20px;">
        ...
        <p>Hi ${name},</p>
        <p>We've received a request to reset your password. Click the button below to set a new password. This link is valid for 10 minutes.</p>
        <div style="text-align:center; margin:30px 0;">
          <a href="${resetUrl}" style="background:#2563eb; color:#ffffff; padding:12px 24px; text-decoration:none; border-radius:5px; font-weight:bold;">
            Reset Password
          </a>
        </div>
